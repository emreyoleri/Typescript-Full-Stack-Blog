            ...
            .then(async (result) => {
                ...
                if (user) {
                    ...
                    if (name) {
                         ...
                         * you are using. Google generraly returns ones, let's
                         * just use that for now
                         */

                        setError('The identiy provider doesn\t have a name.');
                        setAuthenticating(false);
                    }
                } else {
                    setError('The identiy provider is missing a lot of the necessary information. Please try another account or provider');
                    setAuthenticating(false);
                }
            })
            .catch((error) => {
            ...
            <Card>
                ...
                <CardBody>
                    ...
                    <Button block disabled={authenticating} onClick={() => SignInWithSocialMedia(Providers.google)} style={{ backgroundColor: '#ea4335', borderColor: '#ea4335' }}>
                        <i className="fab fa-goggle mr-2"></i> Sign {isLogin ? 'in' : 'up'} with Google
                    </Button>
